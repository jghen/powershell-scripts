...
1. Kan velge å inkludere mappenavn i filnavn (kun for undermapper av FDV-mapper)
2. Ekskluder mapper 200-210, 300-310 .. etc. Dette er ikke FDV-mapper.
3. Kan velge å legge inn prosjektnavn i alle filnavn etter yyyy_bb
4. Fikset problem med omdøping av 80-mappe
...
#hvis ikke 2 eller 3 siffer FDV-mappe -> fila får navnet til mappen over seg. Dette kan være dust og litt risky. lange filnavn?

#nb! tar ikke lange filnavn!!!!!! Eller gjør den det?

function FileInheritsParentDirName {
    ...
    foreach ($file in $files) {
        ...
        $FDVfolder = $is2DigitFolder -Or $is3DigitFolder
        ...
        $countProgress++
        $activity = "Filer arver navnet til mappen over seg (gjelder ikke 2 og 3siffer FDVmappe)..."
        $progress = "Fremdrift: $countFiles av totalt $($files.Count) filer"
        $progressBar = (($countProgress / $files.Count)  * 100)

        Write-Progress -Activity $activity -status $progress -percentComplete $progressBar
    }
    Write-Progress -Activity $activity -Completed
}

if (($inheritFolderName -eq "Y") -Or ($inheritFolderName -eq "y")){
    FileInheritsParentDirName
